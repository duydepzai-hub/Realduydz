-- CẤU HÌNH HỆ THỐNG
getgenv().Key = "duydepzaitonghop_freekeynew"
getgenv().KeyLink = "duydepzaitonghop_freekeynew" -- Thay link của bạn vào đây
...
local function CreateKeyUI(callback)
    ...
    Title.Text = "Duydepzai Hub - Đăng Nhập"
    ...
    CheckButton.BackgroundColor3 = Color3.fromRGB(0, 150, 0)
    
    local CopyButton = Instance.new("TextButton", Frame)
    CopyButton.Text = "Copy Link Get Key"
    ...
    CheckButton.MouseButton1Click:Connect(function()
        if TextBox.Text == getgenv().Key then
            ...
            TextBox.PlaceholderText = "Sai Key rồi!"
            TextBox.Text = ""
        end
    end)
...
Rayfield:Notify({
   ...
   Content = "chào mừng đến với duydepzaitonghop hub",
   Duration = 10,
   Image = 4483362458,
...
AddScript(AntiTab, "Anti Fling", "Chống bọn dùng Fling làm văng", function()
...
VisualTab:CreateButton({
   ...
   Callback = function()
      ...
      game:GetService("Lighting").GlobalShadows = false
      Rayfield:Notify({Title = "Duydepzai Hub", Content = "Đã làm sáng map!", Duration = 2})
   end,
})
...
-- ================= TAB 15: BABFT =================
local BF_Tab = Window:CreateTab("evade", 4483362458)
AddScript(BF_Tab, "gumanba hub(auto cày sk)", "Không có key", function() 
